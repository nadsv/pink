!function() {
    ...
    function replaceIndexes(elem, index) {
        var tmpl = elem.innerHTML, newIndex = parseInt(index) - 1, oldPostfix = index + '"', rexp = new RegExp(oldPostfix, "gm"), newPostfix = newIndex + '"', newTmpl = tmpl.replace(rexp, newPostfix), travelerName = elem.querySelector("#traveler-name-" + index).value, travelerNick = elem.querySelector("#traveler-nick-" + index).value;
        elem.innerHTML = newTmpl, elem.querySelector("#traveler-name-" + newIndex).value = travelerName, 
    ...
    function updateIndexes(index) {
        ...
        for (var travelers = document.getElementsByClassName("traveler"), i = index - 1; i < travelers.length; i++) {
            var oldIndex = i + 2;
    ...
    function deleteRandonTraveler() {
        return function() {
            var index = this.id.substring("del-traveler-".length), removedTraveler = document.querySelector("#traveler-" + index);
            area.removeChild(removedTraveler), updateIndexes(index);
        };
    }
    function add() {
        return function() {
            ...
            delBtns = document.querySelector("#del-traveler-" + count), delBtns.addEventListener("click", deleteRandonTraveler());
        };
    }

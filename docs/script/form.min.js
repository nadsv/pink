!function() {
    ...
    function changeTravelers(input, index, btnClass) {
        "btn--add" === btnClass && (addTravelers(index), 
    ...
    function reduce() {
        return function() {
            var parent = this.parentElement, input = parent.querySelector(".review__input"), oldValue = parseInt(input.value), newValue = oldValue - 1, min = minValue(units(input)), count = min > newValue ? min : newValue;
            input.value = count + " " + units(input);
            var parent = input.parentElement;
            parent.classList.contains("review__field-group--travelers") && changeTravelers(input, oldValue, "btn--reduce"), 
            parent.classList.contains("review__field-group--duration") && setEndDate(picker);
        };
    }
    ...
    function add() {
        return function() {
            ...
            input.value = count + " " + units(input);
            var parent = input.parentElement;
            parent.classList.contains("review__field-group--travelers") && changeTravelers(input, count, "btn--add"), 
            parent.classList.contains("review__field-group--duration") && setEndDate(picker);
        };
    }
    function setEndDate(elem) {
        var beginDate = elem.getMoment(), duration = parseInt(document.getElementById("duration").value) - 1, momentDuration = moment.duration(duration, "d"), endDuration = beginDate.add(momentDuration);
        document.getElementById("end-journey").value = endDuration.format("Do MMMM YYYY");
    }
    for (var MAX_OF_TRAVELERS = 10, MIN_OF_TRAVELERS = 0, MAX_DAYS = 100, MIN_DAYS = 1, area = document.querySelector(".review-fldset--travelers"), reduceBtns = (document.querySelector("#traveler-template"), 
...
function() {
    ...
    function addDeleteEvent(div) {
        div.querySelector(".review__delete-photo").addEventListener("click", function(event) {
    ...
    function formValidate() {
        for (var answer = document.querySelector(".answer--failure"), requiredFields = document.querySelectorAll("[required]"), i = 0; i < requiredFields.length; i++) {
            var input = requiredFields[i];
            if ("" === input.value.trim()) return answer.classList.remove("hidden"), 
            !1;
        }
        return !0;
    }
    if ("FormData" in window) {
        ...
        if (form) {
            ...
            if ("FileReader" in window) {
                ...
                for (var deleteCrosses = document.getElementsByClassName("review__delete-photo"), i = 0; i < deleteCrosses.length; i++) {
...
function() {
    ...
    var btnOK = document.querySelector("#btn-ok"), btnFailure = document.querySelector("#btn-failure"), ansFalilure = document.querySelector(".answer--failure"), ansOK = document.querySelector(".answer--success");
